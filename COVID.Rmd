...
```
```{r}
country_data=coronavirus::coronavirus
...
df1= coronavirus %>%
  ...
  summarise(tot_cases = sum(cases)) %>%
    arrange(date) %>%
    # records daily cumulative cases as cumulative_Cases
    mutate(cumulative_cases = cumsum(tot_cases))%>%
...
country_data <- coronavirus %>% 
  ...
  ungroup()
```
##modify code to reach cases
find total active cases versus cumulative cases
##New task
start with a literate programming paragraph that describes what to do next in our code chunk
track number of active cases (-died and -)
describe how you would get at that in code
cumulate cases, but subtract cumulations of deaths and subtract cumulations of recoveries
--> filter out type "recovered" and "death"
--> filter out deaths per day
--> filter by state
